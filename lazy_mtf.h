...
private:
   struct node_t
   {
      ...
      explicit node_t(T const & data, std::shared_ptr<node_t> next = {}) : data(data), next(next) {}
   };

   void mtf(std::shared_ptr<node_t> & ptr, std::shared_ptr<node_t> & prev)
   {
      ptr->counter = (ptr->counter + 1) % alpha_;
...
public:
   explicit lazy_mtf_t(std::size_t alpha) : sum_cost_(0), alpha_(alpha) {}
